...
export const applyStudentProject = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    const project = await projectsService.applyProject(id, groupId);
...
export const acceptRequestStudentProject = async (req: any, res: any, next: any) => {
  const id = req.params.id;
  const { groupId } = req.body;
  try {
    if (!id) {
    ...
    const project = await projectsService.acceptProjectRequest(id, groupId);
    res.send({ message: `Accepted Request for Project successfully!`, project });
  } catch (ex) {
    next(ex);
  }
};
export const rejectRequestStudentProject = async (req: any, res: any, next: any) => {
  const id = req.params.id;
  const { groupId } = req.body;
  try {
    if (!id) {
    ...
    const project = await projectsService.rejectProjectRequest(id, groupId);
    res.send({ message: `Rejected Request for Project successfully!`, project });
  } catch (ex) {
    next(ex);
  }
};
